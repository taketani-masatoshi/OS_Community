...
export function AdminModulePromotionActions({
  moduleId,
  labels,
}: {
  moduleId: string;
  ...
  async function act(action: "approve" | "reject") {
    setError("");
    const res = await fetch("/api/admin/module-promotions", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "same-origin",
      body: JSON.stringify({ moduleId, action, note: note.trim() || undefined }),
    });
    if (res.ok) {
